...
/*   Updated: 2020/12/03 12:18:54 by ttarsha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_threads(t_input *input)
{
	...
	{
		if ((input->pid[i] = fork()) < 0)
			return ;
		else if (input->pid[i] == 0)
			ft_simulation(&args[i]);
	}
	waitpid(-1, 0, 0);
	i = -1;
	while (++i < input->num_philo)
		kill(input->pid[i], SIGKILL);
	free(args);
}
